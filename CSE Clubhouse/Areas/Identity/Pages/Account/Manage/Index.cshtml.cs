...
namespace CSE_Clubhouse.Areas.Identity.Pages.Account.Manage
{
    public class IndexModel : PageModel
    {
        ...
        private async Task LoadAsync(ClubhouseUser user)
        {
            ...
            Username = userName;
            if (user.BirthDay != null && user.AdmissionYear != null)
            {
                Input = new InputModel
                {
                    ...
                    Position = position,
                    CollegeId = collegeId,
                    AdmissionYear = (DateTime)admissionYear
                };
            }
            else
			{
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            var collegeId = user.CollegeID;
            if (Input.CollegeId != collegeId)
			{
                user.CollegeID = Input.CollegeId;
                await _userManager.UpdateAsync(user);
			}
            var admissionYear = user.AdmissionYear;
            if (Input.AdmissionYear != admissionYear)
			{
                user.AdmissionYear = Input.AdmissionYear;
                await _userManager.UpdateAsync(user);
			}
